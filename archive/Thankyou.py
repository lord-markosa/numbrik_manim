...
class SubscribeEndScreen(Scene):
    def construct(self):
        ...
        button = RoundedRectangle(
            corner_radius=0.2, height=1, width=4, color=RED_N, fill_opacity=1
        ).move_to(TITLE_TEXT_POSITION)

        sub_text = Text(
            "SUBSCRIBE", font="Segoe UI", font_size=42, color=WHITE
        ).next_to(button, 0)
        ...
        subtitle = Text(
            "for more...", font="Verdana", font_size=28, color=NUMBRIK_COLOR
        ).next_to(subscribe_button, DOWN, buff=0.4)
