...
class Quadratics(Scene):
    ...
    def overview(self):
        animateTextSeq(self, [
            ...
            MathTex("a", "x^2", " + b", "x",
                    " + c").set_color_by_tex("x", PURE_GREEN),
        ])
    ...
    def basic_graph(self):
        plane = NumberPlane(
            x_range=[-5, 5, 1], x_length=6,
            y_range=[0, 16, 1], y_length=6,
            background_line_style={
                "stroke_color": YELLOW, "stroke_opacity": 0.3}
        ).add_coordinates().shift(2*LEFT)
        ...
        value_table = Table([*[[str(x), str(y)] for x, y in zip(
            discreteX, discreteY)]], col_labels=[MathTex("x"),
                                                 MathTex("y = x^2")],
            include_outer_lines=True, line_config={
                "stroke_color": YELLOW,
                "stroke_width": 0.5,
                "stroke_opacity": 0.5
        }).shift(4*RIGHT).scale(0.5)

        self.play(Create(value_table.get_rows()[0]), Create(
            value_table.get_vertical_lines()), Create(value_table.get_horizontal_lines()))
    ...
    def graph_parabola(self):
        ...
        plane1 = NumberPlane(
            x_range=[-5, 5, 1], x_length=7,
            y_range=[-1, 16, 2], y_length=6,
            background_line_style={
                "stroke_color": YELLOW, "stroke_opacity": 0.3}
        ).add_coordinates().shift(2*LEFT)

        func1 = always_redraw(lambda: plane1
                              .plot(lambda x: (x-x_0.get_value()) **
                                    2 + y_0.get_value(), x_range=[-12**.5+x_0.get_value(), 12**.5+x_0.get_value()]).set_color(NUMBRIK_COLOR))

        equation_text = MathTex("y = x^2").shift(
            4*RIGHT+UP).set_color(NUMBRIK_COLOR)
        new_equation_text = always_redraw(lambda: MathTex(
            "y = (x -", "x_0", ")^2 +", "y_0")).next_to(equation_text, 0).set_color(NUMBRIK_COLOR)
        new_equation_text[1].set_color(GREEN_E)
        new_equation_text[3].set_color(GREEN_E)

        x_0_label = MathTex(r"x_0 = \qquad").next_to(
            equation_text, 2*DOWN).set_color(GREEN_E)
        y_0_label = MathTex(r"y_0 = \qquad").next_to(
            x_0_label, 1.2*DOWN).set_color(GREEN_E)

        x0_value = always_redraw(
            lambda: DecimalNumber(num_decimal_places=1)
            .set_value(x_0.get_value())
            .next_to(x_0_label, RIGHT, buff=0.1).shift(0.1*RIGHT)
            .set_color(GREEN_E)
        )
        ...
        y0_value = always_redraw(
            ...
            .next_to(y_0_label, RIGHT, buff=0.1).shift(0.1*RIGHT)
            .set_color(GREEN_E)
        )

        vertex = always_redraw(lambda: Dot(color=GREEN_E).move_to(
            plane1.c2p(x_0.get_value(), (y_0.get_value()))))

        self.play(LaggedStart(DrawBorderThenFill(plane1)),
                  Create(func1), Create(equation_text), Create(vertex))
        self.play(Transform(equation_text, new_equation_text))
        self.play(Write(x_0_label), Write(y_0_label),
                  Write(x0_value), Write(y0_value))
    ...
    def animate_factorization_steps(self, lines):
        ...
        self.play(TransformFromCopy(lines[0][0], lines[1][0]), TransformFromCopy(
            lines[0][2], lines[1][2]), run_time=1.5)
    ...
    def factorization_example1(self):
        ...
        factorize_title = MathTex(
            "x^2 +", "5", "x+", "6").to_edge(UP).set_color_by_tex("5", GREEN).set_color_by_tex("6", BLUE)
    ...
    def factorization_example2(self):
        factorize_title = MathTex(
            "x^2", "- x", "-12").to_edge(UP).set_color_by_tex_to_color_map({"-12": BLUE, "- x": GREEN})
        self.play(Write(factorize_title))
        self.wait(1)

        lines1 = animateTextSeq(self, [
            MathTex("-1", "=", "p", "+", "q")
            .set_color_by_tex_to_color_map({"p": ORANGE, "q": YELLOW, "-1": GREEN}),
            MathTex("-12", "=", "p", "\\cdot", "q")
            .set_color_by_tex_to_color_map({"p": ORANGE, "q": YELLOW, "-12": BLUE}),
        ...
        correctSplit = MathTex("-12", "=", "3", "\\cdot",
                               "-4").next_to(lines1[-1], 0)
    ...
    def complete_the_square_example(self):
        ...
        lastEqn1 = animateTextSeq(self, [
            MathTex("x^2 + 5x + 6"),
            MathTex(
                "x^2 + 2\\left(\\frac{5}{2}\\right)x + 6"),
            MathTex(
                "x^2 + 2\\left(\\frac{5}{2}\\right)x", "+ \\left(\\frac{5}{2}\\right)^2", "- \\left(\\frac{5}{2}\\right)^2", "+ 6").set_color_by_tex("\\left(\\frac{5}{2}\\right)^2", RED),
        ...
        justifyingExp2 = MathTex("(a^2 - b^2) = (a - b)(a + b)").next_to(
            lastEqn2[1], RIGHT, buff=0.5).scale(0.8)

        self.play(Write(justifyingExp2))

        finalEqn2 = MathTex(
            "\\left( x + 2 \\right)", "\\left( x + 3 \\right)").next_to(lastEqn2[-1], DOWN, buff=0.5)
