...
class Tut1(Scene):
    def construct(self):
        ...
        box = Rectangle(width=1, height=1, color=BLUE, fill_opacity=0.5,
                        stroke_width=2, stroke_color=ORANGE).move_to(DOWN)
...
class Tut2(Scene):
    def construct(self):
        ...
        circle = always_redraw(lambda: Circle(
            radius=r.get_value(), stroke_color=YELLOW, stroke_width=5))
        ...
        triangle = always_redraw(lambda: Polygon(circle.get_top(), circle.get_left(
        ), circle.get_right(), fill_color=GREEN_C)
        )
...
class Tut3(Scene):
    def construct(self):
        ...
        dot1 = always_redraw(lambda: Dot(
            point=LEFT * 2 + UP, radius=r.get_value()))
